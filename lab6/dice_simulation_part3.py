...
number was rolled is output in a table format. prints expected and difference
"""
...
def print_output_in_table_format(rolls_done, rolls_count_dict):
    ...
    print("\nnumber   picks   percent    expected    difference")

    #  holds the expected values for each number to be accessed for printing and calculation of difference below.
    rolls_expected_dict = {2: (1/36), 3: (1/18), 4: (3/36), 5: (4/36), 6: (5/36), 7: (1/6), 8: (5/36),
                           9: (4/36), 10: (3/36), 11: (1/18), 12: (1/36)}

    rows_printed = 2
    while rows_printed <= 12:

        #  formats the output of the table
        print('{:6}{:8}{:10}{:12}{:14}'.format(rows_printed, rolls_count_dict[rows_printed],
                                               round(rolls_count_dict[rows_printed]/rolls_done, 2),
                                               round(rolls_expected_dict[rows_printed], 2),
                                               (round(rolls_count_dict[rows_printed]/rolls_done - rolls_expected_dict[rows_printed], 2))))
        rows_printed += 1
